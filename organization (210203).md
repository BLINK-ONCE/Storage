...
통계 분야는 우리에게 훈련 세트를 제공한다. 또한 일반화 작업이라는 기계 학습 목표를 달성하는 데 사용할 수 있는 많은 도구를 제공한다. 모수 추정, 편향 및 분산과 같은 기본 개념은 generalization, underfitting, overfittin 개념을 공식적으로 특성화하는 데 유용하다.


## Point Estimation

점 추정은 특정 관심수량에 대한 단일 최상의 예측을 제공하는 것이 목표이다. 일반적으로 관심 수량은 5.1.4절의 선형 회귀 예제의 가중치(weight)와 같은 일부 매개변수 모델에서 단일 매개변수 혹은 매개 변수의 벡터가 될 수 있지만 전체 함수가 될 수도 있다.

모수의 추정치와 참 값을 구별하기 위해, 우리는 보통 다음과 같이 모수의 점 추정치를 나타낸다.

{x1, ... , xm} = m 이라는 각각 독립적이고 동일한 분포의 집합이 있다.
